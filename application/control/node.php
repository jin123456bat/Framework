...
class node extends BaseControl
{
	...
	function cds_cache()
	{
		...
		$result = $feedbackModel->Join('user_info', 'user_info.sn=feedback.sn')->select(array(
			'user_info.sn', // 设备SN号
	...
	function cds()
	{
		...
		if (! empty($group))
		{
			$sns = $this->model('cds_group_sn')
				->where('cds_group_id=?', array(
				$group
			))
				->select('sn');
			foreach ($sns as $sn)
	...
	function detail()
	{
		...
		for ($t_time = $start_time; strtotime($t_time) < strtotime($end_time); $t_time = date('Y-m-d H:i:s', strtotime($t_time) + $duration))
		{
			...
			$avg = $this->model('report_jxreport')
				->where('day=? and sn=?', array(
				$t_time,
				$sn
			))
				->find(array(
	...
	function cds_report()
	{
		...
		{
			$result = $this->model('feedbackHistory')
				->where('ctime>=? and ctime<? and sn=?', array(
				$starttime,
				$endtime,
				$node['sn']
			))
