...
class task extends bgControl
{
	...
	function day()
	{
		...
		// 清理空间
		$this->model('feedbackHistory')
			->where('TIMESTAMPDIFF(MONTH,ctime,now())>=1')
			->delete();
		$this->model('feedbackHistory')->optimize();
		$this->model('task_detail')
			->where('TIMESTAMPDIFF(DAY,createtime,now())>=1')
			->delete();
		$this->model('task_detail')->optimize();
		$this->model('cache')
			->where('TIMESTAMPDIFF(MONTH,createtime,now())>=2')
			->delete();
		$this->model('cache')->optimize();
		$this->model('build_data_log')
			->where('TIMESTAMPDIFF(day,run_starttime,now())>=7')
			->delete();
		$this->model('build_data_log')->optimize();
		$this->model('task_run_log')
			->where('TIMESTAMPDIFF(day,starttime,now())>=7')
			->delete();
