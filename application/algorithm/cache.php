...
class cache extends BaseComponent
{
	...
	private function getDataTime($name, $duration, $defaultStartTime = null)
	{
		...
		if (empty($startTime))
		{
			if (empty($defaultStartTime))
			{
				...
				{
					case 300:
						$startTime = date('Y-m-d H:i:s', strtotime('-14 day'));
					break;
					case 1800:
					case 3600:
					case 7200:
	...
	function operation_stat_algorithm($duration, $startTime, $endTime, $sn = array())
	{
		...
		switch ($duration)
		{
			...
			break;
			case 7200:
				$time = 'concat(date_format(make_time,"%Y-%m-%d "),lpad(floor(date_format(make_time,"%H")/2)*2,2,0),":00:00")';
			break;
			case 24 * 60 * 60:
				$time = 'date_format(make_time,"%Y-%m-%d 00:00:00")';
			break;
			default:
				echo "operation_stat_algorithm中duration错误";
		}
	...
	function traffic_stat($duration, $startTime = null, $endTime = null)
	{
		if (empty($startTime) && empty($endTime))
		{
			...
			$max = array(
				$time_traffic_stat['max'],
				$time_cdn_traffic_stat['max'],
				$time_xvirt_traffic_stat['max']
			);
			$min = array(
				$time_traffic_stat['min'],
				$time_cdn_traffic_stat['min'],
				$time_xvirt_traffic_stat['min']
			);
			
			$result = $this->model('traffic_stat')
				->where('create_time=? and add_time>=? and add_time<?', array(
				$time_traffic_stat['min'],
				$startTime,
				$endTime
			))
			...
			$result = $this->model('cdn_traffic_stat')
				->where('make_time=? and create_time>=? and create_time<?', array(
				$time_cdn_traffic_stat['min'],
				$startTime,
				$endTime
			))
		...
		for ($t_time = $min_time; strtotime($t_time) < strtotime($max_time); $t_time = date('Y-m-d H:i:s', strtotime($t_time) + 86400))
		{
			...
			)))
			{
				...
				$this->model($user_online_tableName)->duplicate(array(
					'online',
					'hit'
				));
	...
	function operation_stat($duration, $startTime = null, $endTime = null)
	{
		if (empty($startTime) && empty($endTime))
		{
			...
			$time = $this->model('operation_stat')
				->where('create_time>=? and create_time<?', array(
				$startTime,
				$endTime
			))
		...
		for ($t_time = $min_time; strtotime($t_time) < strtotime($max_time); $t_time = date('Y-m-d H:i:s', strtotime($t_time) + 86400))
		{
			...
			$operation_stat = $this->operation_stat_algorithm($duration, $startTimeStage, $endTimeStage);
