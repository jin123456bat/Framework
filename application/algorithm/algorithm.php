...
class algorithm extends BaseComponent
{
		...
		else
		{
			for ($t_time = $this->_starttime; strtotime($t_time) < strtotime($this->_endtime); $t_time = date('Y-m-d H:i:s', strtotime($t_time) + $this->_duration))
			{
				if (! empty($sn))
				{
					...
					{
						$this->model('feedbackHistory')->where('sn=?', array(
							$sn
						));
					}
				}
	...
	public function user($sn = array())
	{
		if (empty($sn))
		{
			...
			$temp = $this->formatTimenode($temp, $this->_starttime, $this->_endtime, $this->_duration, array(
				'online' => 0,
				'hit' => 0
			));
			return $temp;
		...
		else
		{
			...
			$user_detail = $this->formatTimenode($user_detail, $this->_starttime, $this->_endtime, $this->_duration, array(
				'online' => 0,
				'hit' => 0
			));
			return $user_detail;
	...
	private function getCategoryName($r)
	{
		...
		{
			case 0:
				$classname = isset($category['http'][$r['category']]) ? $category['http'][$r['category']] : '其他';
			break;
			case 1:
				$classname = isset($category['mobile'][$r['category']]) ? $category['mobile'][$r['category']] : '其他';
			break;
			case 2:
				if ($r['category'] >= 128)
				{
					$classname = isset($category['videoLive'][$r['category'] - 128]) ? $category['videoLive'][$r['category'] - 128] : '其他';
					break;
				}
				else
				{
					$classname = isset($category['videoDemand'][$r['category']]) ? $category['videoDemand'][$r['category']] : '其他';
					break;
				}
		}
		return $classname;
	...
	public function CPService($sn = array(), $top = 9)
	{
		if (is_scalar($sn) || (is_array($sn) && count($sn) == 1))
		{
			...
			$categoryTop = $this->model($tableName)
				->where('time>=? and time<?', array(
				$this->_starttime,
				$this->_endtime
			))
		...
		else
		{
			for ($t_time = $this->_starttime; strtotime($t_time) < strtotime($this->_endtime); $t_time = date('Y-m-d H:i:s', strtotime($t_time) + $this->_duration))
			{
				...
				if (! empty($sn))
				{
					...
					{
						$traffic_stat_model->where('sn=?', array(
							$sn
						));
					}
				}
				...
				if (! empty($sn))
				{
					if (is_scalar($sn))
					{
						$cdn_traffic_stat_model->where('sn like ?', array(
							'%' . substr($sn, 3)
						));
					}
					else if (is_array($sn))
				...
				$cdn_traffic_stat = $cdn_traffic_stat_model->where('make_time>=? and make_time<?', array(
					$t_time,
					date('Y-m-d H:i:s', strtotime($t_time) + $this->_duration)
				))
					->order('time', 'asc')
				...
				if (! empty($sn))
				{
					if (is_scalar($sn))
					{
						$xvirt_traffic_stat_model->where('sn like ?', array(
							'%' . substr($sn, 3)
						));
					}
					else
